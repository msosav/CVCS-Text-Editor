...
LZ77::LZ77() : dictionary() {
    inicializar_diccionario();
}

void LZ77::inicializar_diccionario() {
    for (int i = 0; i < 256; ++i) {
...
void LZ77::comprimir(std::vector<std::string> &content, const std::string& archivo_comprimido) {
...
std::vector<std::string> LZ77::descomprimir(const std::string& archivo_comprimido) {
...
void LZ77::escribir_codigo(int code, std::ofstream &fout) {
    fout.write(reinterpret_cast<const char *>(&code), sizeof(code));
}

void LZ77::mostrarDiccionario()
{
