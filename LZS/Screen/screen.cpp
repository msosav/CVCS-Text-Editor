...
Screen::Screen(const char *file_name, std::vector<std::string> buffer) noexcept
	: rows{}, cols{}, buffer{buffer}, file_name{file_name}, file_info_bar{}
{
	initscr();
	start_color();
...
std::vector<std::string> Screen::get_buffer() const noexcept
{
	return buffer;
}
