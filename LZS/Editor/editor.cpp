...
Editor::Editor(const char *file_name, const std::vector<std::string> &buffer) noexcept
	: screen{file_name, buffer}, lsz(), file_buffers{buffer}
{
	screen.display(std::begin(file_buffers),
				   std::end(file_buffers),
				   cursor);
}

std::vector<std::string> Editor::create_file_buffers(FILE *file) const noexcept
{
	...
	while ((c = fgetc(file)) != EOF)
	{
		...
		{
			file_buffers.push_back(file_line);
			file_line = "";
		}
	}
	return file_buffers;
...
void Editor::process_keypress(int character) noexcept
{
	...
	screen.display(std::begin(file_buffers) + top_of_screen_index,
				   std::end(file_buffers),
				   cursor);
}
...
void Editor::normal_mode_action(int character) noexcept
{
	...
	case 'q':
		endwin();
		lsz.compress(file_buffers, "prueba.bin");
		exit(1);
		break;
	case 'l':
		move_cursor_right();
	...
	case 'h':
		...
		break;
	case 'x':
		file_buffers[file_buffers_index].replace(cursor.row_offset, 1, "");
		break;
	case 'w':
		do_w_motion();
		break;
...
{
	const auto &current_line = file_buffers[file_buffers_index];
	if (file_buffers_index < file_buffers.size() && cursor.row_offset < current_line.size())
	{
...
{
	if (file_buffers_index != 0)
	{
		const auto &line_above = file_buffers[file_buffers_index - 1];
		const auto &current_line = file_buffers[file_buffers_index];
...
{
	if (file_buffers_index < file_buffers.size())
	{
		if (file_buffers_index + 1 < file_buffers.size())
		{
			const auto &current_line = file_buffers[file_buffers_index];
			const auto &line_below = file_buffers[file_buffers_index + 1];
...
{
	std::size_t next_space_index = file_buffers[file_buffers_index]
...
void Editor::delete_char() noexcept
{
	...
	if (file_buffers_index < file_buffers.size())
	{
		...
		auto &current_line = file_buffers[file_buffers_index];
		if (cursor.row_offset == 0 && current_line.size() == 0)
		{
			// Then delete the whole line
			file_buffers.erase(std::begin(file_buffers) + file_buffers_index);
		}
		else
		{
...
void Editor::add_new_line() noexcept
{
	if (file_buffers_index < file_buffers.size())
	{
		...
		file_buffers.insert(
			std::begin(file_buffers) + file_buffers_index + 1,
			rest_of_line);
...
void Editor::insert_char(int character) noexcept
{
	...
	{
		auto &current_line = file_buffers[file_buffers_index];
		if (cursor.row_offset < file_buffers[file_buffers_index].size())
		{
